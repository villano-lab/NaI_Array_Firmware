...
import PuBeTrigger6_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_polarity_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_polarity, handle)
    return err, data

def REG_polarity_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_polarity, handle)
    return err

def REG_thrs_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_thrs, handle)
    return err, data

def REG_thrs_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_thrs, handle)
    return err

def REG_inhib_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_inhib, handle)
    return err, data

def REG_inhib_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_inhib, handle)
    return err

def REG_delay_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_delay, handle)
    return err, data

def REG_delay_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_delay, handle)
    return err

def REG_top_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_top, handle)
    return err, data

def REG_top_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_top, handle)
    return err

def REG_gate_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_gate, handle)
    return err, data

def REG_gate_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_gate, handle)
    return err

def REG_ratereset_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger6_RegisterFile.SCI_REG_ratereset, handle)
    return err, data

def REG_ratereset_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger6_RegisterFile.SCI_REG_ratereset, handle)
    return err
...
def RATE_METER_RateMeter_0_GET_DATA(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, PuBeTrigger6_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def RATE_METER_RateMeter_0_GET_DATA_COUNTS(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, PuBeTrigger6_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS + 512, timeout_ms, handle)
    return err, data, read_data, valid_data
