...
import UT1AReplica_RegisterFile
from ctypes import *
import array
import numpy as np
import os
...
def REG_gate_l_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_gate_l, handle)
    return err, data

def REG_gate_l_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_gate_l, handle)
    return err

def REG_polarity_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_polarity, handle)
    return err, data

def REG_polarity_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_polarity, handle)
    return err

def REG_thrsh_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_thrsh, handle)
    return err, data

def REG_thrsh_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_thrsh, handle)
    return err

def REG_inhib_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_inhib, handle)
    return err, data

def REG_inhib_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_inhib, handle)
    return err

def REG_delay_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_delay, handle)
    return err, data

def REG_delay_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_delay, handle)
    return err

def REG_ratereset_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_ratereset, handle)
    return err, data

def REG_ratereset_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_ratereset, handle)
    return err

def REG_gate_u_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_gate_u, handle)
    return err, data

def REG_gate_u_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_gate_u, handle)
    return err

def REG_top_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_top, handle)
    return err, data

def REG_top_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_top, handle)
    return err

def REG_disable_1_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_disable_1, handle)
    return err, data

def REG_disable_1_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_disable_1, handle)
    return err

def REG_disable_A_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_disable_A, handle)
    return err, data

def REG_disable_A_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_disable_A, handle)
    return err

def REG_disable_B_GET(handle):
    [err, data] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_disable_B, handle)
    return err, data

def REG_disable_B_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_disable_B, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, UT1AReplica_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err
...
def RATE_METER_RateMeter_0_GET_DATA(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, UT1AReplica_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def RATE_METER_RateMeter_0_GET_DATA_COUNTS(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, UT1AReplica_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS + 512, timeout_ms, handle)
    return err, data, read_data, valid_data



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(2048, UT1AReplica_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
