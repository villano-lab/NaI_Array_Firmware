...
import NaIArrayAntonTest6_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_counts_GET(handle):
    [err, data] = __abstracted_reg_read(NaIArrayAntonTest6_RegisterFile.SCI_REG_counts, handle)
    return err, data

def REG_counts_SET(data, handle):
    err = __abstracted_reg_write(data, NaIArrayAntonTest6_RegisterFile.SCI_REG_counts, handle)
    return err
