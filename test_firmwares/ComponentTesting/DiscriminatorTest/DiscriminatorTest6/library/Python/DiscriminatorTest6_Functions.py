...
import DiscriminatorTest6_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_thresh_GET(handle):
    [err, data] = __abstracted_reg_read(DiscriminatorTest6_RegisterFile.SCI_REG_thresh, handle)
    return err, data

def REG_thresh_SET(data, handle):
    err = __abstracted_reg_write(data, DiscriminatorTest6_RegisterFile.SCI_REG_thresh, handle)
    return err
