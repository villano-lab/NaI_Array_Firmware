...
def REG_CN_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_CN, handle)
    return err, data

def REG_CN_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_CN, handle)
    return err

def REG_pol_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_pol, handle)
    return err, data

def REG_pol_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_pol, handle)
    return err

def REG_thrs_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_thrs, handle)
    return err, data

def REG_thrs_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_thrs, handle)
    return err

def REG_inhib_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_inhib, handle)
    return err, data

def REG_inhib_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_inhib, handle)
    return err

def REG_top_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_top, handle)
    return err, data

def REG_top_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_top, handle)
    return err

def REG_delay_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_delay, handle)
    return err, data

def REG_delay_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_delay, handle)
    return err

def REG_gate_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_gate, handle)
    return err, data

def REG_gate_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_gate, handle)
    return err

def REG_gatel_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_gatel, handle)
    return err, data

def REG_gatel_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_gatel, handle)
    return err

def REG_xdelay_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_xdelay, handle)
    return err, data

def REG_xdelay_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_xdelay, handle)
    return err

def REG_inttime_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_inttime, handle)
    return err, data

def REG_inttime_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_inttime, handle)
    return err

def REG_preint_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_preint, handle)
    return err, data

def REG_preint_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_preint, handle)
    return err

def REG_pileup_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_pileup, handle)
    return err, data

def REG_pileup_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_pileup, handle)
    return err

def REG_gain_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_gain, handle)
    return err, data

def REG_gain_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_gain, handle)
    return err

def REG_ofs_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_ofs, handle)
    return err, data

def REG_ofs_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_ofs, handle)
    return err

def REG_bl_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_bl, handle)
    return err, data

def REG_bl_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_bl, handle)
    return err

def REG_noflip0flip1_GET(handle):
    [err, data] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_noflip0flip1, handle)
    return err, data

def REG_noflip0flip1_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_noflip0flip1, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, test5_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err



def SPECTRUM_Spectrum_0_RESET(handle):
    err = __abstracted_reg_write(2, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_START(handle):
    err = __abstracted_reg_write(4, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_FLUSH(handle):
    err = __abstracted_reg_write(1, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_STOP(handle):
    err = __abstracted_reg_write(0, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err
...
def SPECTRUM_Spectrum_0_SET_PARAMETERS(Bin, AcquisitionMode, TargetValue, handle):
    ...
    if (Bin == 128):
        rebin_value= 7
    err = __abstracted_reg_write(rebin_value, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG_REBIN, handle)
    acq_mode=0
    if (AcquisitionMode =="Event"):
        acq_mode=(1 << 30) + TargetValue
    if (AcquisitionMode =="Time"):
        acq_mode=(1 << 31) + TargetValue*1000
    err = __abstracted_reg_write(acq_mode, test5_RegisterFile.SCI_REG_Spectrum_0_CONFIG_LIMIT, handle)
    return err

def SPECTRUM_Spectrum_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_Spectrum_0_STATUS, handle)
    return err, status

def SPECTRUM_Spectrum_0_GET_DATA(Bin, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(Bin, test5_RegisterFile.SCI_REG_Spectrum_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, test5_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(2048, test5_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def LISTMODULE_Digitizer_0_RESET(handle):
    err = __abstracted_reg_write(2, test5_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    return err

def LISTMODULE_Digitizer_0_START(handle, channels):
    err = __abstracted_reg_write(2 + (channels<<8), test5_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(0 + (channels<<8), test5_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1 + (channels<<8), test5_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_Digitizer_0_SetLen(handle, length):
    err = __abstracted_reg_write(length, test5_RegisterFile.SCI_REG_Digitizer_0_ACQ_LEN, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_Digitizer_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_Digitizer_0_STATUS, handle)
    status = status & 0xf
    return err, status

def LISTMODULE_Digitizer_0_GET_DATA(Data_Number, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_fifo_read(Data_Number, test5_RegisterFile.SCI_REG_Digitizer_0_FIFOADDRESS, test5_RegisterFile.SCI_REG_Digitizer_0_STATUS,1, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def CPACK_CP_0_RESET(handle):
	err = __abstracted_reg_write(2, test5_RegisterFile.SCI_REG_CP_0_CONFIG, handle)
	err = __abstracted_reg_write(0, test5_RegisterFile.SCI_REG_CP_0_CONFIG, handle)
	return err

def CPACK_CP_0_FLUSH(handle):
	err = __abstracted_reg_write(4, test5_RegisterFile.SCI_REG_CP_0_CONFIG, handle)
	err = __abstracted_reg_write(0, test5_RegisterFile.SCI_REG_CP_0_CONFIG, handle)
	return err
...
def CPACK_CP_0_START(handle):
	...
	if (err != 0):
	   return False
	err = __abstracted_reg_write(1, test5_RegisterFile.SCI_REG_CP_0_CONFIG, handle)
	if (err != 0):
	   return False
	return True

def CPACK_CP_0_GET_STATUS(handle):
	[err, status] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_CP_0_READ_STATUS, handle)
	status = status & 0xf
	return err, status

def CPACK_CP_0_GET_AVAILABLE_DATA(handle):
	[err, status] = __abstracted_reg_read(test5_RegisterFile.SCI_REG_CP_0_READ_VALID_WORDS, handle)
	return err, status

def CPACK_CP_0_GET_DATA(n_packet, timeout_ms, handle):
	data_length = n_packet *( 3 + <<<NUMBER OF PACKET LINES AFTER THE HEADER HERE>>>)
	[err, data, read_data, valid_data] = __abstracted_fifo_read(data_length, test5_RegisterFile.SCI_REG_CP_0_FIFOADDRESS, test5_RegisterFile.SCI_REG_CP_0_READ_STATUS, True, timeout_ms, handle)
	return err, data, read_data, valid_data
