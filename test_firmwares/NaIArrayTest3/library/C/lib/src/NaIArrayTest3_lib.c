...
SCILIB int ClearBuffer(void *buffer_handle)
{
	...
	return 0;
}SCILIB int REG_integrals3_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_integrals3, handle);
}
SCILIB int REG_integrals3_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_integrals3, handle);
}
