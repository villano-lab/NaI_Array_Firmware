...
void print_usage(FILE* stream, int exit_code){ //This looks unaligned but lines up correctly in the terminal output
	...
	" -r,	--reduce	<value>		When triggering, skip each <value> trigger to reduce the overall rate.\n"
...
{
	skip = -1; //count will never be -1, so if no skip argument is provided we don't skip anything
	...
	int iarg=0;
	while(iarg != -1){
		iarg = getopt_long(argc, argv, "+d:r:l::shv::V", longopts, &index);
	...
	if(verbose>0){printf("Skipping every %d entries.\n",skip);}
	skip_q = REG_skip_SET(skip,&handle);
	if(verbose>1){printf("Skip set successfully? error code: %d\n",skip_q);}
