...
import Integration_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_Cutoff_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Cutoff, handle)
    return err, data

def REG_Cutoff_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Cutoff, handle)
    return err

def REG_Gate_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Gate, handle)
    return err, data

def REG_Gate_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Gate, handle)
    return err

def REG_Integral_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Integral, handle)
    return err, data

def REG_Integral_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Integral, handle)
    return err

def REG_Reset_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Reset, handle)
    return err, data

def REG_Reset_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Reset, handle)
    return err

def REG_Delay_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Delay, handle)
    return err, data

def REG_Delay_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Delay, handle)
    return err

def REG_Counts_GET(handle):
    [err, data] = __abstracted_reg_read(Integration_RegisterFile.SCI_REG_Counts, handle)
    return err, data

def REG_Counts_SET(data, handle):
    err = __abstracted_reg_write(data, Integration_RegisterFile.SCI_REG_Counts, handle)
    return err
