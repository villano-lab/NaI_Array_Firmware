from TrigTest8_Functions import *
from ctypes import *
...
if (count > 0):
    ...
    plt.ion()
    if (SPECTRUM_Spectrum_NoDelay_SET_PARAMETERS(Spectrum_bin, Spectrum_acq_mode, Spectrum_target_value, handle) != 0): 
        print("Set Parameters Error")    
    if (SPECTRUM_Spectrum_NoDelay_RESET(handle) != 0): 
        print("Reset Error")
    if (SPECTRUM_Spectrum_NoDelay_FLUSH(handle) != 0): 
        print("Flush Error")
    if (SPECTRUM_Spectrum_NoDelay_START(handle) == 0):
        while 1:
            [err, Spectrum_Status] = SPECTRUM_Spectrum_NoDelay_GET_STATUS(handle)
            [err, Spectrum_Data, Spectrum_Read_Data, Spectrum_Valid_Data] = SPECTRUM_Spectrum_NoDelay_GET_DATA(Spectrum_bin, Timeout_ms, handle)
            for i in range(Spectrum_bin):
                Spectrum[i] = Spectrum_Data[i] 
            plt.plot(Spectrum)
            plt.draw()
