...
import ANSK029amp2_RegisterFile
from ctypes import *
import array
import numpy as np
import os
...
def REG_pol_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_pol, handle)
    return err, data

def REG_pol_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_pol, handle)
    return err

def REG_ofs_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_ofs, handle)
    return err, data

def REG_ofs_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_ofs, handle)
    return err

def REG_thrs_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_thrs, handle)
    return err, data

def REG_thrs_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_thrs, handle)
    return err

def REG_TK_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_TK, handle)
    return err, data

def REG_TK_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_TK, handle)
    return err

def REG_TM_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_TM, handle)
    return err, data

def REG_TM_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_TM, handle)
    return err

def REG_K_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_K, handle)
    return err, data

def REG_K_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_K, handle)
    return err

def REG_decovM_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_decovM, handle)
    return err, data

def REG_decovM_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_decovM, handle)
    return err

def REG_M_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_M, handle)
    return err, data

def REG_M_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_M, handle)
    return err

def REG_gain_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_gain, handle)
    return err, data

def REG_gain_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_gain, handle)
    return err

def REG_blen_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_blen, handle)
    return err, data

def REG_blen_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_blen, handle)
    return err

def REG_blinib_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_blinib, handle)
    return err, data

def REG_blinib_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_blinib, handle)
    return err

def REG_samplepos_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_samplepos, handle)
    return err, data

def REG_samplepos_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_samplepos, handle)
    return err

def REG_runcfg_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_runcfg, handle)
    return err, data

def REG_runcfg_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_runcfg, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK029amp2_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(6144, ANSK029amp2_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def SPECTRUM_Spectrum_0_RESET(handle):
    err = __abstracted_reg_write(2, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_START(handle):
    err = __abstracted_reg_write(4, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_FLUSH(handle):
    err = __abstracted_reg_write(1, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_STOP(handle):
    err = __abstracted_reg_write(0, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err
...
def SPECTRUM_Spectrum_0_SET_PARAMETERS(Bin, AcquisitionMode, TargetValue, handle):
    ...
    if (Bin == 128):
        rebin_value= 7
    err = __abstracted_reg_write(rebin_value, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG_REBIN, handle)
    acq_mode=0
    if (AcquisitionMode =="Event"):
        acq_mode=(1 << 30) + TargetValue
    if (AcquisitionMode =="Time"):
        acq_mode=(1 << 31) + TargetValue*1000
    err = __abstracted_reg_write(acq_mode, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_CONFIG_LIMIT, handle)
    return err

def SPECTRUM_Spectrum_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_STATUS, handle)
    return err, status

def SPECTRUM_Spectrum_0_GET_DATA(Bin, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(Bin, ANSK029amp2_RegisterFile.SCI_REG_Spectrum_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
