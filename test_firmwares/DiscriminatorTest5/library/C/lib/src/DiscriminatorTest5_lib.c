...
#include  "DiscriminatorTest5_lib.h"
...
SCILIB int ClearBuffer(void *buffer_handle)
{
	...
	return 0;
}SCILIB int REG_thresh_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_thresh, handle);
}
SCILIB int REG_thresh_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_thresh, handle);
}
