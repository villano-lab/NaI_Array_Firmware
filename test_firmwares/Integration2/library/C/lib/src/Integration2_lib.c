...
SCILIB int REG_Cutoff_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_Cutoff, handle);
}
SCILIB int REG_Cutoff_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_Cutoff, handle);
}
...
{
     return __abstracted_reg_read(val, SCI_REG_Integral, handle);
}
