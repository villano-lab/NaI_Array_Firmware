...
SCILIB int REG_flip_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_flip, handle);
}
SCILIB int REG_flip_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_flip, handle);
}
...
SCILIB int CPACK_FIFO_START(NI_HANDLE *handle)

{
return __abstracted_reg_write(1,SCI_REG_FIFO_CONFIG, handle);
...
SCILIB int CPACK_FIFO_STOP(NI_HANDLE *handle)

{
return __abstracted_reg_write(0,SCI_REG_FIFO_CONFIG, handle);
...
SCILIB int CPACK_FIFO_RESET(NI_HANDLE *handle)
{
return __abstracted_reg_write(0,SCI_REG_FIFO_CONFIG, handle);
...
SCILIB int CPACK_FIFO_FLUSH(NI_HANDLE *handle)
{
return __abstracted_reg_write(0,SCI_REG_FIFO_CONFIG, handle);
...
SCILIB int CPACK_FIFO_STATUS(uint32_t *status,NI_HANDLE *handle)
{
int err;
uint32_t int_status=0;
err= __abstracted_reg_read(&int_status, SCI_REG_FIFO_READ_STATUS, handle);
...
SCILIB int CPACK_FIFO_DATA_AVAILABLE(uint32_t *status,NI_HANDLE *handle)
{
return __abstracted_reg_read(status, SCI_REG_FIFO_READ_VALID_WORDS, handle);
...
SCILIB int CPACK_FIFO_DOWNLOAD(uint32_t *val, uint32_t size, int32_t timeout, NI_HANDLE *handle, uint32_t *read_data, uint32_t *valid_data)
{
return __abstracted_fifo_read(val, size, SCI_REG_FIFO_FIFOADDRESS, SCI_REG_FIFO_READ_STATUS,1, timeout, handle, read_data, valid_data);

}
//-----------------------------------------------------------------
//-
//- CPACK_FIFO_RECONSTRUCT_DATA
//-
...
SCILIB int CPACK_FIFO_RECONSTRUCT_DATA(void *buffer_handle, t_generic_event_collection *decoded_packets)
{
	cbuf_handle_t cbuf;
	cbuf = (cbuf_handle_t)buffer_handle;
	int PacketSize =3;
	...
	while (circular_buf_size(cbuf)> 0)
	{
		...
		if (in_sync == 2) {
			temp_data.row[1]  =  mpe;
			decoded_packets->packets[k].payload = malloc(sizeof(t_FIFO_struct));
			if (decoded_packets->packets[k].payload != NULL) {
			    *((t_FIFO_struct*)decoded_packets->packets[k].payload) = temp_data;
			    k++;
