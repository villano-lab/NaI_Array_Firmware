...
/* //REMOVE THIS COMMENT TO ENABLE THE EXAMPLE CODE
	...
	if (CPACK_FIFO_RESET(&handle) != 0) printf("Reset Error");
	if (CPACK_FIFO_START(&handle) != 0) printf("Start Error");
	if (CPACK_FIFO_STATUS(&status_frame, &handle) != 0) printf("Status Error");
	if (status_frame >0)
	{
		while (1)
		{
			valid_data_frame = 0;
			if (CPACK_FIFO_DOWNLOAD(&data_frame, N_Packet * (<<<PACKET_SIZE_HERE_IN_WORK>>>), timeout_frame, &handle, &read_data_frame, &valid_data_frame) != 0) printf("Data Download Error");
			...
			if (CPACK_FIFO_RECONSTRUCT_DATA(BufferDownloadHandler, &decoded_packets) == 0)
			{
				printf(".");
