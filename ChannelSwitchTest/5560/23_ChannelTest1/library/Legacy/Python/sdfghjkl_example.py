from sdfghjkl_Functions import *
from ctypes import *
...
*/
    ...
    ReadDataNumber = 0
    if (CPACK_FIFO_RESET(handle) != 0):
        print("Reset Error!")
    if (CPACK_FIFO_START(handle) == True):
        [err, Frame_Status] = CPACK_FIFO_GET_STATUS(handle)
        if (Frame_Status >0):
            while(ReadDataNumber < N_Total_Events):
                [err, Frame_Data, Frame_Read_Data, Frame_Valid_Data] = CPACK_FIFO_GET_DATA(N_Packet, Timeout_ms, handle)
                [Time_Code, Pack_Id, Energy] = CPACK_FIFO_RECONSTRUCT_DATA(Frame_Data)                 
