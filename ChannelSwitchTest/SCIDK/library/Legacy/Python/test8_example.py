...
if (count > 0):
    ...
    LISTMODULE_Digitizer_1_SetLen(handle, WaveformLen) 

    fp = open('data.hex','wb')
    if (LISTMODULE_Digitizer_1_START(handle, ChannelsEnable) == True):
        while(ReadDataNumber < TargetDataNumber):
            [err, List_Data, List_Read_Data, List_Valid_Data] = LISTMODULE_Digitizer_1_GET_DATA(size_list, Timeout_ms, handle)    
