...
SCILIB int LISTMODULE_Digitizer_1_RESET(NI_HANDLE *handle)

{
return __abstracted_reg_write(2,SCI_REG_Digitizer_1_CONFIG, handle);

}
//-----------------------------------------------------------------
//-
//- LISTMODULE_Digitizer_1_START
//-
...
SCILIB int LISTMODULE_Digitizer_1_START(NI_HANDLE *handle, int nchannels)

{
int r1 = __abstracted_reg_write(2+ (nchannels << 8),SCI_REG_Digitizer_1_CONFIG, handle);
int r2 = __abstracted_reg_write(0+ (nchannels << 8),SCI_REG_Digitizer_1_CONFIG, handle);
int r3 = __abstracted_reg_write(1+ (nchannels << 8),SCI_REG_Digitizer_1_CONFIG, handle);
if ((r1 == 0) && (r2 == 0) && (r3 == 0))
    return 0;
else
    return -1;
...
SCILIB int LISTMODULE_Digitizer_1_SetLen(NI_HANDLE *handle, int length)

{
int r1 = __abstracted_reg_write(length,SCI_REG_Digitizer_1_ACQ_LEN, handle);
if (r1 == 0)
    return 0;
else
    return -1;
...
SCILIB int LISTMODULE_Digitizer_1_STATUS(uint32_t *status, NI_HANDLE *handle)

{
int err;
uint32_t int_status=0;
err= __abstracted_reg_read(&int_status, SCI_REG_Digitizer_1_STATUS, handle);
*status = int_status & 0xF;
return err;
...
//-----------------------------------------------------------------
//-
//- LISTMODULE_Digitizer_1_DOWNLOAD
//-
//- Download data from fifo buffer. If data are not available the command stall until the timout occuredUSAGE: 
//-     LISTMODULE_Digitizer_1_DOWNLOAD(data_buffer, BUFFER_SIZE_Digitizer_1, 1000, handle, rd, vp);
//- 
//-
//- ARGUMENTS:
//- 	             val  PARAM_OUT   uint32_t
...
SCILIB int LISTMODULE_Digitizer_1_DOWNLOAD(uint32_t *val, uint32_t size, int32_t timeout, NI_HANDLE *handle, uint32_t *read_data, uint32_t *valid_data)

{
return __abstracted_fifo_read(val, size, SCI_REG_Digitizer_1_FIFOADDRESS, SCI_REG_Digitizer_1_STATUS,1, timeout, handle, read_data, valid_data);
