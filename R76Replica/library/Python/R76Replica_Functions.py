...
def REG_skip_GET(handle):
    [err, data] = __abstracted_reg_read(R76Replica_RegisterFile.SCI_REG_skip, handle)
    return err, data

def REG_skip_SET(data, handle):
    err = __abstracted_reg_write(data, R76Replica_RegisterFile.SCI_REG_skip, handle)
    return err

def REG_count_GET(handle):
    [err, data] = __abstracted_reg_read(R76Replica_RegisterFile.SCI_REG_count, handle)
    return err, data

def REG_count_SET(data, handle):
    err = __abstracted_reg_write(data, R76Replica_RegisterFile.SCI_REG_count, handle)
    return err

def REG_skip_attempts_GET(handle):
    [err, data] = __abstracted_reg_read(R76Replica_RegisterFile.SCI_REG_skip_attempts, handle)
    return err, data

def REG_skip_attempts_SET(data, handle):
    err = __abstracted_reg_write(data, R76Replica_RegisterFile.SCI_REG_skip_attempts, handle)
    return err

def REG_eq_count_GET(handle):
    [err, data] = __abstracted_reg_read(R76Replica_RegisterFile.SCI_REG_eq_count, handle)
    return err, data

def REG_eq_count_SET(data, handle):
    err = __abstracted_reg_write(data, R76Replica_RegisterFile.SCI_REG_eq_count, handle)
    return err
...
def RATE_METER_RateMeter_Reduced_GET_DATA(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, R76Replica_RegisterFile.SCI_REG_RateMeter_Reduced_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data




def RATE_METER_RateMeter_Reduced_GET_DATA_COUNTS(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, R76Replica_RegisterFile.SCI_REG_RateMeter_Reduced_FIFOADDRESS + 512, timeout_ms, handle)
    return err, data, read_data, valid_data
