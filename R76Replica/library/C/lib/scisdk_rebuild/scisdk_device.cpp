...
NI_RESULT SciSDK_Device::Connect() {
	...
	if (_JSONFwFilePath != "") {
		try {
			...
			if (ifile) {
				...
				BuildTree(jcfg, "");
				std::cout << "_hal->Connect...\n";
				NI_RESULT res = _hal->Connect(_DevicePath, _DeviceModel);
				std::cout << "_hal->Connected?\n";
				if (res == NI_OK) {
					std::cout << "Success!\n";
					connected = true;
				}else {
					std::cout << "Connection failed with code " << res << std::endl;
					return res;
				}
			}else {
				std::cout << "JSON not found at provided path!\n";
	...
	std::cout << "JSON path provided was: " << _JSONFwFilePath << ". Returning NI_OK. \n";
	return NI_OK;
}
