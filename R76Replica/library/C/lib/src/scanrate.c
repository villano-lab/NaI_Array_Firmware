...
int main(int argc, char* argv[])
{
	...
	inhib_q = __abstracted_reg_write(inhib,SCI_REG_trig_inhib,(NI_HANDLE*)sdk);	//Set number of samples to delay data by
	delay_q = __abstracted_reg_write(delay,SCI_REG_trig_delay,(NI_HANDLE*)sdk);	//Set number of samples to delay data by
	gate_uq = __abstracted_reg_write(gate_u,SCI_REG_trig_gate_u,(NI_HANDLE*)sdk);
	gate_lq = __abstracted_reg_write(gate_l,SCI_REG_trig_gate_l,(NI_HANDLE*)sdk);
	polarity_q = __abstracted_reg_write(polarity,SCI_REG_trig_polarity,(NI_HANDLE*)sdk);	//Set polarity to negative
	...
	while(thrs < range_u){
		...
		for(i = 0; i<wait; i++){
			...
			rate_q=RATE_METER_RateMeter_0_GET_DATA(rateval,ratechan,ratetimeout, (NI_HANDLE*)sdk, &rateread_data, &ratevalid_data);
