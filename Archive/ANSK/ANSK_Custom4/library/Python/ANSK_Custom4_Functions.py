...
import ANSK_Custom4_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_thrs_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_thrs, handle)
    return err, data

def REG_thrs_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_thrs, handle)
    return err

def REG_inib_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_inib, handle)
    return err, data

def REG_inib_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_inib, handle)
    return err

def REG_polarity_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_polarity, handle)
    return err, data

def REG_polarity_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_polarity, handle)
    return err

def REG_inttime_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_inttime, handle)
    return err, data

def REG_inttime_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_inttime, handle)
    return err

def REG_pre_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_pre, handle)
    return err, data

def REG_pre_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_pre, handle)
    return err

def REG_pileup_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_pileup, handle)
    return err, data

def REG_pileup_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_pileup, handle)
    return err

def REG_gain_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_gain, handle)
    return err, data

def REG_gain_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_gain, handle)
    return err

def REG_ofs_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_ofs, handle)
    return err, data

def REG_ofs_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_ofs, handle)
    return err

def REG_read_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_read, handle)
    return err, data

def REG_read_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_read, handle)
    return err

def REG_energy_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_energy, handle)
    return err, data

def REG_energy_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_energy, handle)
    return err

def REG_empty_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_empty, handle)
    return err, data

def REG_empty_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_empty, handle)
    return err

def REG_baseline_GET(handle):
    [err, data] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_baseline, handle)
    return err, data

def REG_baseline_SET(data, handle):
    err = __abstracted_reg_write(data, ANSK_Custom4_RegisterFile.SCI_REG_baseline, handle)
    return err



def SPECTRUM_Spectrum_0_RESET(handle):
    err = __abstracted_reg_write(2, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_START(handle):
    err = __abstracted_reg_write(4, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_FLUSH(handle):
    err = __abstracted_reg_write(1, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_STOP(handle):
    err = __abstracted_reg_write(0, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err
...
def SPECTRUM_Spectrum_0_SET_PARAMETERS(Bin, AcquisitionMode, TargetValue, handle):
    ...
    if (Bin == 128):
        rebin_value= 7
    err = __abstracted_reg_write(rebin_value, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG_REBIN, handle)
    acq_mode=0
    if (AcquisitionMode =="Event"):
        acq_mode=(1 << 30) + TargetValue
    if (AcquisitionMode =="Time"):
        acq_mode=(1 << 31) + TargetValue*1000
    err = __abstracted_reg_write(acq_mode, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_CONFIG_LIMIT, handle)
    return err

def SPECTRUM_Spectrum_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_STATUS, handle)
    return err, status

def SPECTRUM_Spectrum_0_GET_DATA(Bin, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(Bin, ANSK_Custom4_RegisterFile.SCI_REG_Spectrum_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(2048, ANSK_Custom4_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
