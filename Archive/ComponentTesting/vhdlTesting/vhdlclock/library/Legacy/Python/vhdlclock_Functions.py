...
def REG_reset_GET(handle):
    [err, data] = __abstracted_reg_read(vhdlclock_RegisterFile.SCI_REG_reset, handle)
    return err, data

def REG_reset_SET(data, handle):
    err = __abstracted_reg_write(data, vhdlclock_RegisterFile.SCI_REG_reset, handle)
    return err
