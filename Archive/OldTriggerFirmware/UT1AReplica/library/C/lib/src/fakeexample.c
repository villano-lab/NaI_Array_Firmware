...
int main(){
	...
	//including the connect-to-board function at all causes the program to segfault before reaching main
	//USB2_ConnectDevice("13251",&handle);
	//if(USB2_ConnectDevice(BOARD_SERIAL_NUMBER, &handle) != 0) { printf("Unable to connect to the board!\n"); return (-1); };
	//seems to be in the connectUSB function -- which is part of the SCIDK-SDKLinux repo
	//SCIDK_ConnectUSB(BOARD_SERIAL_NUMBER,&handle);
	//SCIDK_ConnectUSB("13251",&handle);
	char sn[255] = "13251";
	printf("%s\n",sn);
	//OpenDeviceBySerialNumber(sn);
	SCIDK_ConnectUSB(sn,&handle);
	return 0;
}
