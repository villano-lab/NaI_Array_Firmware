...
/* //REMOVE THIS COMMENT TO ENABLE THE EXAMPLE CODE
	...
	if (SPECTRUM_Spectrum_NoDelay_SET_PARAMETERS(Rebin, LimitMode, LimitValue, &handle) != 0) printf("Set Parameters Error");
	if (SPECTRUM_Spectrum_NoDelay_FLUSH(&handle) != 0) printf("Flush Error");
	if (SPECTRUM_Spectrum_NoDelay_RESET(&handle) != 0) printf("Reset Error");
	if (SPECTRUM_Spectrum_NoDelay_START(&handle) == 0) {
		if (SPECTRUM_Spectrum_NoDelay_STATUS(&status_spectrum, &handle) == 0) {
			while (status_spectrum != 0)
			{
				if (SPECTRUM_Spectrum_NoDelay_DOWNLOAD(&data_spectrum, bins, timeout_spectrum, &handle, &read_data_spectrum, &valid_data_spectrum) == 0)
				{
					for (int i = 0; i < 16384; i++)
						spectrum[i] = data_spectrum[i];
				}
			...
			if (SPECTRUM_Spectrum_NoDelay_STOP(&handle) != 0) printf("Stop Error");
		}
	}
	else printf("Start Error");
*/
